...
// Editable up until NRS transmission, not just until payment -- being paid
// doesn't "seal" an invoice the way actually being transmitted to NRS does
// (nothing has -- see the mark-paid/void routes' own comments on why this
// changed from the original "locked once a receipt exists" rule). Editing
// a paid invoice does mean its already-issued receipt (a snapshot copied
// at Mark Paid time) can end up disagreeing with the edited invoice --
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (invoice.fetch_transmission_status === "Sent") {
    return NextResponse.json({ error: "Can't edit an invoice already transmitted to NRS." }, { status: 400 });
  }
